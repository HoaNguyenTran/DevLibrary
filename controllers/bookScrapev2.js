...
(async (genre) => {
  ...
  for (var p = 1; p <= 3; p++) {
    let link = await bookScrapev2(
      page,
      `https://www.fahasa.com/sach-trong-nuoc/tam-ly-ky-nang-song/tam-ly.html?order=num_orders&limit=2&p=${p}`
    );
    links = links.concat(link);
  }
  console.log(links);
  for (link in links) {
    let book = await getBook(page, links[link], "business");
    await books.push(book);
  }
  console.log(books);
  await browser.close();
  await Book.deleteMany({ genre: "business" });
  await Book.insertMany(books);
});


// Run node bookScrapev2.js
